...
```typescript
import { retry, createRetry, withRetry, TimeoutError } from '@the-south-pacific/async';
...
Attempting to import from `@the-south-pacific/colors` or `@the-south-pacific/strings` will result in a linting error due to module boundary constraints.
